...
if __name__ == "__main__":
    ...
    analyze_color(guesses=guesses, corrects=corrects, i=0, assume_correct=assume_correct, assume_incorrect=assume_incorrect)
    round += 1
    others = [c for c in color_set if c not in assume_correct and c not in assume_incorrect]
    ...
    while corrects[round - 1] < 4:
        # remaining = [c for c in color_set if c not in assume_correct and c not in assume_incorrect]
        # next_guess = assume_correct + random.sample(remaining, 4 - corrects[round - 1])
        next_guess = assume_correct + others[:4-len(assume_correct)]
        if next_guess == guesses[round - 1]:
            next_guess = assume_correct + others[len(assume_correct)-4:len(others)]
        # if len(next_guess) < 4:
        #     for c in others:
        #         if (next_guess + [c]) != guesses[round - 1]:
        #             next_guess += [c]
        #         if len(next_guess) == 4: break
